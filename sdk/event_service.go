...
func (e EventService) GetResponseData(endpoint string) ([]byte, error) {
	...
	if err != nil {
		return []byte{}, fmt.Errorf("failed to get data: %w", err)
	}
	defer resp.Body.Close()

	b, err := ioutil.ReadAll(resp.Body)
	if err != nil {
		return []byte{}, fmt.Errorf("failed to get response body: %w", err)
	}

	if resp.StatusCode < http.StatusOK || resp.StatusCode >= http.StatusMultipleChoices {
		return []byte{}, fmt.Errorf("request failed, status code - %d, response - %s", resp.StatusCode, string(b))
	}
	return b, nil
}
...
func parseResponseBody(response []byte, dest interface{}) error {
